...
## [v3.0.0-alpha] - 2026-03-31

### Added
- Rational timeline (`src/timeline.rs`) for deterministic absolute time accumulation.
- Stateful cursor (`src/cursor.rs`) to track octave, duration, and velocity per voice.
- Euclidean rhythm generator (`src/euclidean.rs`) for algorithmic beat generation.
- Spelling engine and lexer with passing tests.

### Changed
- The core compiler is now stable and all tests pass.
