...
describe('Concrete Audio Engine Compliance', () => {
  it('parses style=concrete and evaluates granular slicing and stretch modifiers into the IR timeline', async () => {
    ...
    console.log(JSON.stringify(result.ast.measures[0].parts[0].voices[0].events, null, 2)); expect(sliceEvents.length).toBe(1); // Half note sliced into 8 discrete chunks
